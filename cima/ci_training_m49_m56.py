...
from helper.monitor import get_focal_area_pos
...
class ConvergenceModel:
            ...
            for i, fp in enumerate(file_list):
                try:
                    ...
                    if not valid_df.empty:
                        ...
                        if self.reduce_fps_flag is not None:
                            # Reduce FPS from 30 (recorded) to 10 (actual running)
                            base_fps, target_fps = self.reduce_fps_flag['base'], self.reduce_fps_flag['target']
    ...
    def load_ex_dataset(self, dir_path=None):
        try:
            ...
            self.dataset_dict['y_train'] = pd.read_csv(os.path.join(dir_path, "y_train.csv"), header=None)
            self.dataset_dict['y_train'] = self.dataset_dict['y_train'].squeeze()

            self.dataset_dict['x_val'] = pd.read_csv(os.path.join(dir_path, "x_val.csv"))
            self.dataset_dict['y_val'] = pd.read_csv(os.path.join(dir_path, "y_val.csv"), header=None)
            self.dataset_dict['y_val'] = self.dataset_dict['y_val'].squeeze()

            self.dataset_dict['x_test'] = pd.read_csv(os.path.join(dir_path, "x_test.csv"))
            self.dataset_dict['y_test'] = pd.read_csv(os.path.join(dir_path, "y_test.csv"), header=None)
            self.dataset_dict['y_test'] = self.dataset_dict['y_test'].squeeze()
    ...
    def _train_model(self, use_hptuning=False):
        ...
        # --- Common params for training the model ---
        ...
        self.logger.log(f"Use noise with STD: {self.noise_std}")

        # Get best model from HP-tuning or user-define model
        if use_hptuning:
            self.logger.log(f"\nRunning Randomized Search CV ({self.n_iter_search} iterations)...")
            best_params = self._train_model_with_hptuning(
            ...
            self.model = lgb.LGBMRegressor(
                learning_rate=0.0039599357466946754,
        ...
        def learning_rate_decay(current_iter):
            initial_lr = 0.039599357466946754
            decay_rate = 0.99
            th, step = 100, 10
    ...
    def _train_model_with_hptuning(self, *, x_train, y_train, x_val, y_val, groups, scoring, sample_weight):
        param_dist = {
            'num_leaves': sp_randint(31, 200),  # Control tree complexity
            'max_depth': sp_randint(6, 32),            # Explicitly limit tree depth
            'learning_rate': loguniform(0.0001, 0.02),  # Smaller learning rates
            'min_child_samples': sp_randint(100, 500),  # Min samples in a leaf
            'reg_alpha': loguniform(0.1, 100.0),  # L1 regularization
            'reg_lambda': loguniform(0.01, 10.0),  # L2 regularization
            'max_bin': sp_randint(32, 255),
            ...
            'subsample_freq': sp_randint(1, 5),  # Bagging frequency if subsample < 1.0
        }

        early_stopping_callback = lgb.early_stopping(stopping_rounds=100, verbose=False, min_delta=0.01)
        fit_callbacks = [
        ...
        self._log_fit_process(random_search)
    ...
    def _log_fit_process(self, search_model):
        """
        ...
        try:
            ...
            n_splits = search_model.n_splits_  # Get actual number of splits used
            n_candidates = len(cv_results['params'])
            scoring_metric = search_model.scoring if isinstance(search_model.scoring, str) else 'score'
            ...
            self.logger.log(str(search_model.cv_results_.keys()))
        except Exception as e:
    ...
    def _plot_predicted_vs_actual(self, x_input, y_true, y_pred, set_name, split_by_focal=False):
        ...
        Each unique (focal_area_x, focal_area_y) gets a unique base color.
        Brightness of each point is modulated by y_true (darker for lower values, brighter for higher).
        If split_by_focal is True, creates a grid of subplots, one for each unique focal area,
        ...
        try:
            focal_pairs = list(zip(x_input['focal_area_x'], x_input['focal_area_y']))
            ...
            if not split_by_focal:
                ...
                n_cols = min(3, n_focals)
                n_rows = int(np.ceil(n_focals / n_cols))
                fig, axes = plt.subplots(n_rows, n_cols, figsize=(5 * n_cols, 5 * n_rows), squeeze=False)
                fig.suptitle(f'Predicted vs Actual ({self.target_col}) by Focal Area on {set_name} Set', fontsize=18)
    ...
    def predict(self, input_data):
        ...
        Args:
            input_data (dict or pd.DataFrame): Input data containing features.
                                            If dict, assumes a single sample.
                                            If DataFrame, predicts for each row.

        Returns:
            float or np.ndarray: Predicted target value (float if input was dict)
                                 or array of predicted values (if input was DataFrame).
        ...
        try:
            ...
            # Ensure output format matches input format expectation
            if is_dict_input:
                if predictions.ndim == 1 and len(predictions) == 1:
                    return predictions[0]  # Return single float value
                else:
                    # Should not happen with single row input, but handle defensively
                    self.logger.log(
                        f"Warning: Expected single prediction for dict input, but got shape {predictions.shape}. Returning array.")
                    return predictions
            else:
                return predictions  # Return numpy array for DataFrame input

        except Exception as e:
            self.logger.log(f"Error during model prediction step: {e}")
            # Re-raise as a RuntimeError to indicate a failure in the prediction process
...
def add_focal_area(df: pd.DataFrame):
    if 'x' in df.columns and 'y' in df.columns:
        focal_x = df['x']
        focal_y = df['y']
    elif 'xLeft' in df.columns and 'yLeft' in df.columns and 'xRight' in df.columns and 'yRight' in df.columns:
        focal_x = np.ceil((df['xLeft'] + df['xRight']) / 2).astype(int)
        focal_y = df['yLeft']
    else:
        print('Error: failed to add_focal_xy')
        return []

    screen_grid = (1920, 1080, 3, 3)
    focal_area_x = []
    focal_area_y = []
    for x, y in zip(focal_x, focal_y):
        col_index, row_index = get_focal_area_pos(x, y, *screen_grid)
        focal_area_x.append(col_index)
        focal_area_y.append(row_index)

    df['focal_area_x'] = focal_area_x
    df['focal_area_y'] = focal_area_y
    return ['focal_area_x', 'focal_area_y']


def add_vergence_angle(df: pd.DataFrame):
    left_x, left_y, left_z = df['left_gaze_vector_x'], df['left_gaze_vector_y'], df['left_gaze_vector_z']
    right_x, right_y, right_z = df['right_gaze_vector_x'], df['right_gaze_vector_y'], df['right_gaze_vector_z']

    # Stack components to form N x 3 arrays for each vector
    v1 = np.stack((left_x, left_y, left_z), axis=-1)
    v2 = np.stack((right_x, right_y, right_z), axis=-1)
    ...
    # Compute cosine of angles, clip for numerical stability
    cos_angles = np.clip(dots / (norms1 * norms2), -1.0, 1.0)

    # Compute angles in degrees
    angles = np.degrees(np.arccos(cos_angles))

    df['vergence_angle'] = np.round(angles, 2)
    return ['vergence_angle']


ext_feature_func_dict = {
    # 'focal_xy[pixels]': add_focal_xy,
    'focal_point': add_focal_area,
    # 'vergence_angle[deg]': add_vergence_angle,
}

# --- Configuration (Copied from request & adapted) ---
CI_PREDICTION_FEATURE_COLS = [
    "left_pupil_x[]", "left_pupil_y[]", "left_pupil_z[]",
    "left_pupil_scr_x[pixels]", "left_pupil_scr_y[pixels]",
    "left_proj_point_x[]", "left_proj_point_y[]", "left_proj_point_z[]",
    "left_proj_point_scr_x[pixels]", "left_proj_point_scr_y[pixels]",
    ...
    "right_pupil_scr_x[pixels]", "right_pupil_scr_y[pixels]",
    "right_proj_point_x[]", "right_proj_point_y[]", "right_proj_point_z[]",
    "right_proj_point_scr_x[pixels]", "right_proj_point_scr_y[pixels]",
...
CI_VALID_COL = 'valid[]'
CI_TARGET_COL = 'd[mm]'
CI_MODEL_SAVE_DIR = "convergence_model_final"


# --- Main Execution Function (Modified as requested) ---
def ci_model_train(user_hp_dict=None):
    ...
    # EXT_DATASET_DIR = "convergence_model_final_v8_20250601_0950"
    ...
    model = ConvergenceModel(
        ...
        reduce_fps_flag={'base': 30, 'target': 10},
        ####################
        noise_std=3 if user_hp_dict is None else user_hp_dict['noise_std'],
        history_depth=0 if user_hp_dict is None else user_hp_dict['history_depth'],
        use_sample_weight=True if user_hp_dict is None else user_hp_dict['use_sample_weight'],
        ####################
        include_general_files=True,
        include_divergence_files=False,
...
# ---- Code For Training Only!!! ----
if __name__ == "__main__":
    offset = 48
    already_done = [49, 50, 51]
    ci_model_base_save_dir = "convergence_model_final_v8_20250607_m"
    list_noise_std = [1.5, 3]
    list_history_depth = [10, 15]
    list_use_sample_weight = [False, True]

    for i, (history_depth, use_sample_weight, noise_std,) in enumerate(
            itertools.product(list_history_depth, list_use_sample_weight, list_noise_std)
    ):
        m = i + 1 + offset
        if m in already_done:
            continue

        print(f"m{m:02d}: noise_std={noise_std},\thistory_depth={history_depth},\tuse_sample_weight={use_sample_weight}")
        CI_MODEL_SAVE_DIR = ci_model_base_save_dir + str(m)
